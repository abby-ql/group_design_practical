...
function App() {
  ...
  type BatchScore = 12 | 41 | 77;
  const scoreByKey: Record<string, BatchScore> = {
    "1": 12,
    "2": 41,
    "3": 77,
  };
  ...
  const addNewItem = () => {
    ...
    setUploadData(prev => [...prev, newItem]);
  };
  ...
  const submitUploadData = async (score: BatchScore) => {
    if (uploadData.length === 0) {
      setStatus("No items to upload");
      return;
    }

    setStatus(`Uploading items with score ${score}...`);
    try {
      const response = await fetch('/risk/items', {
        ...
        body: JSON.stringify(
          uploadData.map(item => ({
            ...
            visibility: 'public',
            language: 'en',
            item_metadata: { manual_score: score }
          }))
        ),
      });
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      const score = scoreByKey[event.key];
      if (!score) return;

      event.preventDefault();
      setAwaitingScore(false);
      void submitUploadData(score);
    };

    window.addEventListener("keydown", onKeyDown);
    return () => window.removeEventListener("keydown", onKeyDown);
  }, [awaitingScore, submitUploadData]);
